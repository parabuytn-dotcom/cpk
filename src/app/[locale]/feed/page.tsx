...
export default async function FeedPage({
  ...
  const canPostImage =
    isFullAdmin(profile.role) || profile.role === "teacher" || profile.tags.includes("feed_publisher");
  const canPostVideo =
    isFullAdmin(profile.role) || profile.role === "teacher" || profile.tags.includes("reels_publisher");
        ...
        <div className="flex flex-col gap-6">
          {posts.map((post) => (
            <PostCard
              key={post.id}
              post={post}
              canDelete={isFullAdmin(profile.role) || profile.id === post.authorId}
            />
          ))}
        </div>
      )}
