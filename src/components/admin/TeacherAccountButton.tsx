...
export default function TeacherAccountButton({ teacherId }: { teacherId: string }) {
  ...
  function handleClick() {
    startTransition(async () => {
      const res = await createTeacherAccount(teacherId);
      setResult(res);
    });
  }
  ...
  if (result?.success) {
    return (
      <div className="rounded-xl border border-brand-500/30 bg-brand-500/10 px-4 py-3 text-sm">
        <p className="font-medium">{t("createdNoteSimple")}</p>
